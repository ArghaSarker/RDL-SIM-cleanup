...
class Train_RDL_Denoising(tf.keras.Model):
    def __init__(self, srmodel, denmodel, loss_fn, optimizer,  parameters, PSF = 'given', verbose = True):
        ...
        if self.PSF is None:
            ...
            self.PSF, self.OTF = create_psf(self.sigma_x, 
                        ...
                        self.dkx, 
                        self.dky)
        elif self.PSF is 'given': # read out the PSF from the RDL_Sim pakage
    ...
    def _get_cur_k(self, image_gt):
        ...
        #print(f'cur_k0 :: {cur_k0} {type(cur_k0)}, cur_k0_angle :: {cur_k0_angle} {type(cur_k0_angle)}, modamp :: {modamp} {type(modamp)}')
    ...
    def fit(self, data, data_val):
        ...
        if self.verbose:
            ...
            plot_batches(input_MPE_batch, input_PFE_batch, gt_batch) # have to pass epoch
